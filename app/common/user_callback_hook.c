...
/* 参照NDK文档内开启IPv6支持
 * 下面三个函数对应于 SysConfig 里的NDK Stack配置中的User Callbacks 。*/
...
void netIPAddrHook(uint32_t IPAddr, unsigned int IfIdx, unsigned int fAdd)
{
     ...
     * 这里的 ti_net_SlNet_initConfig 是通过SysConfig添加网卡时自动生成的，这里替换成CC3120 初始化。 */
    ...
    if (fAdd && createTask) {
        /*
         *  从这里开始开启一个主线程用于 CC3120 Booster初始化，并且开始MQTT的相关
         */
