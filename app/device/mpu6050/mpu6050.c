...
//#define  I2C2_REG HWREG(I2C2_BASE + I2C_O_MCS)


void MPU_DelayMs(uint32_t num){
//    tickPeriod = SysTickPeriodGet() ;
    //SysCtlDelay((systemClock / 3000) * num);
    // freertos 使用vTaskDelay.
    vTaskDelay(num);
}

u8 MPU_WriteByte(uint8_t device_address,u8 reg,u8 data)
{
//    return MPU_WriteData(device_address,reg,1,&data);
    uint8_t writeBuffer[2];
    writeBuffer[0] = reg;
    writeBuffer[1] = data;
    write_bytes(device_address,writeBuffer,2);
//    write_bytes(device_address,&reg,1);
//    write_bytes(device_address,&data,1);
    return 0;
}

u8 MPU_ReadByte(uint8_t device_address,u8 reg)
{
//    I2C_WriteByte(device_address,reg); //写寄存器地址
//    MPU_DelayMs(10000);
//    return I2C_ReadByte(device_address);
    // 使用simple link driver
    uint8_t res =0 ;
...
//    I2C2_Init();
    ...
    MPU_WriteByte(MPU_ADDR,MPU_PWR_MGMT2_REG,0X00);  //唤醒MPU6050

    MPU_SetGyroFsr(3);                    //陀螺仪传感器,±2000dps
    MPU_SetAccelFsr(0);                   //加速度传感器,±2g
    MPU_SetRate(50);                       //设置采样率50Hz
    MPU_WriteByte(MPU_ADDR,MPU_INT_EN_REG,0X00);    //关闭所有中断
    MPU_WriteByte(MPU_ADDR,MPU_USER_CTRL_REG,0X00); //I2C主模式关闭
    MPU_WriteByte(MPU_ADDR,MPU_FIFO_EN_REG,0X00);   //关闭FIFO
    MPU_WriteByte(MPU_ADDR,MPU_INTBP_CFG_REG,0X80); //INT引脚低电平有效
    res=MPU_ReadByte(MPU_ADDR,MPU_DEVICE_ID_REG);
    printf("read MPU_DEVICE_ID_REG %x\r\n",res);
    if(res==MPU_ADDR)//器件ID正确
    {
        MPU_WriteByte(MPU_ADDR,MPU_PWR_MGMT1_REG,0X01); //设置CLKSEL,PLL X轴为参考
        MPU_WriteByte(MPU_ADDR,MPU_PWR_MGMT2_REG,0X00); //加速度与陀螺仪都工作
        //DelayUs(3000);
        MPU_SetRate(50);                       //设置采样率为50Hz
    }else return 1;
    return 0;
}
...
{
    return MPU_WriteByte(MPU_ADDR,MPU_GYRO_CFG_REG,fsr<<3);//设置陀螺仪满量程范围
}

u8 MPU_SetAccelFsr(u8 fsr)
{
    return MPU_WriteByte(MPU_ADDR,MPU_ACCEL_CFG_REG,fsr<<3);//设置加速度传感器满量程范围
}
...
u8 MPU_SetLPF(u16 lpf)
{
    ...
    return MPU_WriteByte(MPU_ADDR,MPU_CFG_REG,data);//设置数字低通滤波器
}
...
u8 MPU_SetRate(u16 rate)
{
    ...
    if(rate<4)rate=4;
    data=1000/rate-1;
    data=MPU_WriteByte(MPU_ADDR,MPU_SAMPLE_RATE_REG,data);  //设置数字低通滤波器
    return MPU_SetLPF(rate/2); //自动设置LPF为采样率的一半
}
...
{
//    I2C_WriteData(slave_address,reg,buf,len);
...
{
//    I2C_ReadData(slave_address,reg,buf,len);
